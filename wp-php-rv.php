...
{
	/**
	 * This server is running PHP vX.x+?
	 *
	 * @return boolean `TRUE` if running PHP vX.x+.
	 */
	function wp_php_rv() // Called automatically on `include()` or `require()`.
	{
...
if(empty($GLOBALS['wp_php_rv']) || !is_string($GLOBALS['wp_php_rv']))
	 ...
	 */
	$GLOBALS['wp_php_rv'] = '5.3'; // Default value.
/*
 * @return boolean on `include()` or `require()`.
 */
return wp_php_rv(); // `TRUE` if running PHP vX.x+; `FALSE` otherwise.